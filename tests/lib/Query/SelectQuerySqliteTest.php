...
final class SelectQuerySqliteTest extends TestCase
{
  ...
  public function test_select_with_group_by(): void
  {
    ...
    $expected = "SELECT users.status AS users_status, COUNT(*) AS total FROM users GROUP BY users.status ORDER BY total DESC;";
    $this->assertSame($expected, $query->generateSql());
  }
  ...
  public function test_select_full_combo(): void
  {
    ...
    $expectedSQL = 'SELECT * FROM users WHERE status = :status_1 ORDER BY created_at DESC LIMIT :limit_1;';
